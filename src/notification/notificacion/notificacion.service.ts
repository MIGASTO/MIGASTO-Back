...
@Injectable() export class NotificacionService { 
  constructor( 
  ...
  async create(dto: CreateNotificacionDto): Promise<Notificacion> { 
    ...
    const notificacion = this.notificacionRepository.create({ ...dto, usuario, }); 
    return await this.notificacionRepository.save(notificacion); } 
    
    
  async findAll(): Promise<Notificacion[]> { 
    return await this.notificacionRepository.find(
      { relations: ['usuario'], order: { fecha_creacion: 'DESC' }, 
    }); } 
    
    
  async findOne(id: number): Promise<Notificacion> {
    const notificacion = await this.notificacionRepository.findOne({ 
      where: { id_notificacion: id }, relations: ['usuario'],
    }); 
    if (!notificacion) { 
  ...
  async update(id: number, dto: UpdateNotificacionDto): Promise<Notificacion> { 
    ...
    if (dto.id_usuario) { 
      ...
      notificacion.usuario = usuario; } Object.assign(notificacion, dto); 
      return await this.notificacionRepository.save(notificacion); 
    } 
