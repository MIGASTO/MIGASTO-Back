 ...
 async update(id: number, updateDto: UpdateMovimientoDto, user: Usuario) {
    ...
    if (updateDto.monto !== undefined) {
        ...
        if (abonoAsociado) {
            ...
            const nuevoTotalPagadoPrestamo = Number(prestamo.monto_pagado) + diferencia;
...
if (updateDto.tags !== undefined) {
      ...
      if (updateDto.tags && updateDto.tags.length > 0) {
        tagsEntities = await this.tagRepo.find({ where: { id_tag: In(updateDto.tags) } });
        if (tagsEntities.length !== updateDto.tags.length) {
           throw new NotFoundException('Uno o más tags enviados no fueron encontrados');
        }
      }
      ...
      if (esAbono) {
        ...
        if (tagAbono) {
            const yaEstaIncluido = tagsEntities.some(t => t.id_tag === tagAbono.id_tag);
            if (!yaEstaIncluido) {
